...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        action = None
        #Use np.random.random() to get a value in [0, 1)
        prob = np.random.random()
        if prob < epsilon:
            #Act random
            action = np.random.choice(all_actions)
        else:
            #Choose max Q (greedy choice)
            max_q = -math.inf
            action = -1
            for i in all_actions:
                if Q[state, i] > max_q:
                    #Update max_q and action
                    max_q = Q[state, i]
                    action = i
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.1
        ...
        # It is recommended you use the np.random module
        action = None
        annealer = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = annealer.value(current_total_steps)
        prob = np.random.random()
        if prob < epsilon:
            #Act random
            action = np.random.choice(all_actions)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        Q = np.random.rand(ns, na) #np.random.rand returns element between 0 and 1
        # ADD YOUR CODE SNIPPET BETWEEN EX. 2.1
        ...
        while episode <= self.episode_max and not diff < self.threshold: #and not diff < self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, list_pos, steps, self.epsilon_initial, self.epsilon_final, self.annealing_timesteps, self.eps_type)
                ...
                for a in pos_mov_s_next:
                    if Q[s_next, a] > max_q:
                       #Update q and ind
                        max_q = Q[s_next, a]
                        max_ind = a
                #Update Q with the best value
                Q[s_current, action] = (1-lr)*Q[s_current, action] + lr*(R + discount*max_q)
            ...
            Q_diff = np.subtract(Q, Q_old)
            #print(Q_diff)
            diff = np.absolute(np.nanmean(Q_diff))
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class ScheduleLinear(object):
    def __init__(self, schedule_timesteps, final_p, initial_p=1.0):
        ...
        self.diff = self.final_p - self.initial_p

    def value(self, t):
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
        # Return the annealed linear value
        annealed_e = self.initial_p + (self.diff* (t/self.schedule_timesteps))
        return annealed_e
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
